...
!c-----
    subroutine eof(m,cov,Eval)
        integer::m
        real:: er(m,4)
        real:: cov(m,m),Eval(m),s(m,m),d(m) !work array
!c---- Eigenvalues and eigenvectors by the Jacobi method 
      call jacobi(m,cov,s,d,0.00001)
!c---- Specified eigenvalues 
      call arrang(m,d,s,er)
      cov=s
      Eval(:)=er(:,1)
    return
    end subroutine
...
!c       ecof(mnl,n): time coefficients of egvt 
      ...
      real:: f(m,n),s(mnl,mnl),er(mnl,4),egvt(m,mnl),ecof(mnl,n)
